...
17. [softcut](docs/engines/softcut.md): a dual-deck crossfaded **overdub looper** on monome's [softcut-lib](https://github.com/monome/softcut-lib) (4 voices, 2 per deck). Unlike the buffer-tape engines, softcut plays and records the same loop at once with subsample-accurate click-free crossfades and **interpolated overdub** — layering live input onto a running loop with an ENV-knob feedback control (sound-on-sound). PITCH = bipolar varispeed/reverse, POS/SIZE = loop window, ENV = overdub feedback, the FLUX pad = a sweepable filter, MOD_AMT = loop crossfade time; Alt+Play arms overdub, Seq realigns all voices (click-free `cutToPos` sync), Alt+PITCH loads loop clips from SD. A normal **SRAM build** (the softcut DSP is tiny); 4 voices is the hardware-measured CPU budget. The vendored core lives in `src/engine/softcut/vendor`
