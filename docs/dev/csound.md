...
The orchestra is loaded from the card when `/csound/patch.csd` is present (a full CSD document), else it is the compiled-in `kOrchestra` in `csound_engine.cpp`: a `vco2` saw with knob1→pitch, knob2→level, knob3→cutoff, triggered by `schedule(1,0,100000)`. An SD patch should name the same control channels (`speedA`/`mixA`/`sizeA`/`envA` and the `B` deck) so the platform knobs drive it; see `channel_for()`. The centre mode LED shows the source: cyan = SD patch, white = built-in.
