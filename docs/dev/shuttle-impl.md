...
- **Verified:** host suite (`make -C host test-shuttle`, 9 groups) green; `make ENGINE=shuttle` builds + links on target. SRAM_EXEC ~82%, SDRAM arena reserved whole (static).

- **Files:** `src/engine/shuttle/shuttle_engine.{h,cpp}`, `host/test_shuttle.cpp`. Platform: the SD-load path reuses the streaming service via the `SPK_USE_STREAM` capability flag (see below).
...
- **Audio path** (`process`, the ISR): render all four tracks; each deck = its two tracks summed at their per-track volume; then per-deck pan x A/B mix-fader -> stereo `out`. All four play at once.
...
`host/test_shuttle.cpp` (9 groups, `make -C host test-shuttle`): speed map; record + unity playback; reverse-wrap + silence-at-noon + loop; Play->unity snap (one reseed); Rev swap + track independence; SD load via a fake stream; Seq re-align (all four atomic); realign declick (no large output step across the jump); POS/SIZE loop window. Uses a controllable `FakeClock` (to step past the 300 ms pad debounce) and a `FakeStream` (to drive the load drain).
