...
namespace softcut {
	...
	// sk-engines PORT NOTE: upstream uses std::atomic<phase_t> (phase_t=double). A 64-bit atomic on
	// the Cortex-M7 is not lock-free and emits __atomic_store_8/__atomic_load_8 libcalls that newlib
	// (nano.specs) does not provide -> link error. Narrowed to std::atomic<float>: 32-bit is naturally
	// lock-free on the M7 (single-instruction load/store, no libcall) and still tear-free for the
	// audio-ISR-writes / main-loop-reads split these fields exist for. Precision: getSavedPosition
	// already returns float and divides by sampleRate, so float frame-phase is ample.
	std::atomic<float> rawPhase;
        std::atomic<float> quantPhase;
