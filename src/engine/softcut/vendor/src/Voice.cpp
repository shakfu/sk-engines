...
void Voice:: processBlockMono(const float *in, float *out, int numFrames) {
    ...
    rawPhase.store(static_cast<float>(sch.getActivePhase()), std::memory_order_relaxed);
...
void Voice::updateQuantPhase() {
    if (phaseQuant == 0) {
        quantPhase.store(static_cast<float>(sch.getActivePhase() / sampleRate), std::memory_order_relaxed);
    } else {
	const phase_t tmp = (sch.getActivePhase() + phaseOffset) / (sampleRate *phaseQuant);
        quantPhase.store(static_cast<float>(std::floor(tmp) * phaseQuant), std::memory_order_relaxed);
    }
}
